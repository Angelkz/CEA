...
class HorarioCarreraForm(forms.ModelForm):
	class Meta:
			...
			fields = ('Serie','FK_Periodo','FK_Semestre','FK_Carrera',)
			widgets = {'Serie': TextInput(attrs={'class': 'form-control','placeholder':'Ingrese Serie,Ejemplo: "SC3B"','title':'Serie','required':'true','size': 10,' style' : ' width:  220px'}),
...
class HorarioCarreraDelForm(forms.Form):
	...
	class Meta:
	    model = HorarioCarrera
	    fields = '__all__'
	    widgets = {'Serie': Select(attrs={'class':'form-control','style':'width: 15em;','title':'Periodo','required':'true'}),}
	    labels ={ 'Serie':_('Serie'),}
